...
abstract class AbstractClearingReviewContentTaskHandler extends AbstractClearingProcessTaskHandler {
  ...
  public function modifyTask(
    ...
    if ($clearingProcess->getReviewerContentContactId() !==
      $previousClearingProcess->getReviewerContentContactId()
    ) {
      $task->setAssigneeContactIds($this->getAssigneeContactIds($clearingProcessBundle->getClearingProcess()));
      $changed = TRUE;
    }

    if (NULL !== $clearingProcess->getIsReviewContent()) {
  ...
  protected function getTaskSubject(ClearingProcessEntityBundle $clearingProcessBundle): string {
    return E::ts('Review Clearing (content)');
  }
  ...
  private function getAssigneeContactIds(ClearingProcessEntity $clearingProcess): array {
    return NULL === $clearingProcess->getReviewerContentContactId()
      ? [] : [$clearingProcess->getReviewerContentContactId()];
  }
  ...
  private function getRequiredPermissions(): array {
    return [ClearingProcessPermissions::REVIEW_CONTENT];
  }
