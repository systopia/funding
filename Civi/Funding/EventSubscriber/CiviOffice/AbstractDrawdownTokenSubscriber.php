...
abstract class AbstractDrawdownTokenSubscriber extends AbstractTokenSubscriber {
  ...
  public function __construct(CiviOfficeContextDataHolder $contextDataHolder) {
    parent::__construct($this->getTokenEntityName(), [
  ...
  public function onCiviOfficeTokenContext(GenericHookEvent $event): void {
    if ($this->getCiviOfficeEntityName() === $event->entity_type) {
      /**
       * @phpstan-var array{
      ...
      $event->context['fundingDrawdownDocument'] = [
  ...
  public function checkActive(TokenProcessor $processor): bool {
    return in_array('fundingDrawdownDocument', $processor->context['schema'] ?? [], TRUE)
      || [] !== $processor->getContextValues('fundingDrawdownDocument');
  }
  ...
  public function evaluateToken(TokenRow $row, $entity, $field, $prefetch = NULL): void {
    ...
    $bankAccount = $row->context['fundingDrawdownDocument']['bankAccount'];
  ...
  abstract protected function getCiviOfficeEntityName(): string;

  abstract protected function getTokenEntityName(): string;
