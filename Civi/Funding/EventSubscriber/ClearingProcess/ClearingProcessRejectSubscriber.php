...
/**
 * Rejects all clearing items when clearing is rejected.
 */
class ClearingProcessRejectSubscriber implements EventSubscriberInterface {
  ...
  public function onUpdated(ClearingProcessUpdatedEvent $event): void {
    ...
    if ('rejected' === $newStatus && 'rejected' !== $oldStatus) {
      $clearingProcessId = $event->getClearingProcess()->getId();
      $this->clearingCostItemManager->rejectByClearingProcessId($clearingProcessId);
      $this->clearingResourcesItemManager->rejectByClearingProcessId($clearingProcessId);
    }
  }
