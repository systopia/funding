 ...
 * @codeCoverageIgnore
 */
final class DefaultFundingCaseFormUpdateValidateHandler implements FundingCaseFormUpdateValidateHandlerInterface {
  ...
  public function handle(FundingCaseFormUpdateValidateCommand $command): FundingCaseValidationResult {
    $handler = $this->serviceLocatorContainer->get($command->getFundingCaseType()->getName())
      ->getFundingCaseFormUpdateValidateHandler();
    if (NULL === $handler) {
      throw new \RuntimeException(sprintf(
        'Funding case type "%s" does not allow to update a funding case',
