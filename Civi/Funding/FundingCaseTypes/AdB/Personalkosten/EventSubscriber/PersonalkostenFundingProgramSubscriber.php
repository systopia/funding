...
namespace Civi\Funding\FundingCaseTypes\AdB\Personalkosten\EventSubscriber;
...
class PersonalkostenFundingProgramSubscriber implements EventSubscriberInterface {
  ...
  public function onPreFundingProgram(PreEvent $event): void {
    if (
      'edit' !== $event->action
      || (!isset($event->params['funding_program_adb.foerderquote'])
        && !isset($event->params['funding_program_adb.sachkostenpauschale']))
    ) {
      return;
    }
    ...
    $fundingProgramGet = FundingProgram::get(FALSE)
      ->addWhere('id', '=', $id)
      ->setSelect([
        'funding_program_adb.foerderquote',
        'funding_program_adb.sachkostenpauschale',
      ])
      ->addJoin(
      ...
      ->addJoin(
        ...
        ['funding_case_type.name', '=', '"' . AdbPersonalkostenMetaData::NAME . '"'],
      );
    ...
    $previousFoerderquote = $fundingProgram['funding_program_adb.foerderquote'];
    if (isset($event->params['funding_program_adb.foerderquote'])) {
      Assert::integerish($event->params['funding_program_adb.foerderquote']);
      $newFoerderquote = $event->params['funding_program_adb.foerderquote'] =
        (int) $event->params['funding_program_adb.foerderquote'];
    ...
    $previousSachkostenpauschale = (float) $fundingProgram['funding_program_adb.sachkostenpauschale'];
    if (isset($event->params['funding_program_adb.sachkostenpauschale'])) {
      Assert::numeric($event->params['funding_program_adb.sachkostenpauschale']);
      $newSachkostenpauschale = $event->params['funding_program_adb.sachkostenpauschale'] =
        round((float) $event->params['funding_program_adb.sachkostenpauschale'], 2);
  ...
  public function onPostFundingProgram(PostEvent $event): void {
    ...
    $fundingCaseBundles = $this->fundingCaseManager->getBundleBy(CompositeCondition::new('AND',
      ...
      Comparison::new('funding_case_type_id.name', '=', AdbPersonalkostenMetaData::NAME)
    ));
