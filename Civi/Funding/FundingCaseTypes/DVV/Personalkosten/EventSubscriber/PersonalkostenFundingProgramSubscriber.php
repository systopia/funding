...
namespace Civi\Funding\FundingCaseTypes\DVV\Personalkosten\EventSubscriber;
...
class PersonalkostenFundingProgramSubscriber implements EventSubscriberInterface {
  ...
  public function onPreFundingProgram(PreEvent $event): void {
    if (
      'edit' !== $event->action
      || (!isset($event->params['funding_program_dvv.foerderquote'])
        && !isset($event->params['funding_program_dvv.sachkostenpauschale']))
    ) {
      return;
    }
    ...
    $fundingProgramGet = FundingProgram::get(FALSE)
      ->addWhere('id', '=', $id)
      ->setSelect([
        'funding_program_dvv.foerderquote',
        'funding_program_dvv.sachkostenpauschale',
      ])
      ->addJoin(
      ...
      ->addJoin(
        ...
        ['funding_case_type.name', '=', '"' . DvvPersonalkostenMetaData::NAME . '"'],
      );
    ...
    $previousFoerderquote = $fundingProgram['funding_program_dvv.foerderquote'];
    if (isset($event->params['funding_program_dvv.foerderquote'])) {
      Assert::integerish($event->params['funding_program_dvv.foerderquote']);
      $newFoerderquote = $event->params['funding_program_dvv.foerderquote'] =
        (int) $event->params['funding_program_dvv.foerderquote'];
    ...
    $previousSachkostenpauschale = (float) $fundingProgram['funding_program_dvv.sachkostenpauschale'];
    if (isset($event->params['funding_program_dvv.sachkostenpauschale'])) {
      Assert::numeric($event->params['funding_program_dvv.sachkostenpauschale']);
      $newSachkostenpauschale = $event->params['funding_program_dvv.sachkostenpauschale'] =
        round((float) $event->params['funding_program_dvv.sachkostenpauschale'], 2);
  ...
  public function onPostFundingProgram(PostEvent $event): void {
    ...
    $fundingCaseBundles = $this->fundingCaseManager->getBundleBy(CompositeCondition::new('AND',
      ...
      Comparison::new('funding_case_type_id.name', '=', DvvPersonalkostenMetaData::NAME)
    ));
