...
namespace Civi\Funding\PayoutProcess\EventSubscriber;
...
final class DrawdownSubscriber implements EventSubscriberInterface {
  ...
  public function onDeleted(DrawdownDeletedEvent $event): void {
    $payoutProcess = $event->getPayoutProcess();
    $payoutProcess->setModificationDate(new \DateTime(\CRM_Utils_Time::date('Y-m-d H:i:s')));
    $this->payoutProcessManager->update($event->getDrawdownBundle());
  }
  ...
  public function onUpdated(DrawdownUpdatedEvent $event): void {
    ...
    if (NULL !== $drawdown->getAcceptionDate()
      ...
      $newModificationDate = new \DateTime(\CRM_Utils_Time::date('Y-m-d H:i:s'));
    }
