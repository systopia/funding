...
/**
 * Directive to display the config for new funding case permissions.
 */
fundingModule.directive('fundingNewCasePermissions', ['$compile', function($compile) {
  return {
    restrict: 'E',
    scope: {
    ...
    link: function(scope, element) {
      ...
      scope.$watch('type', function (newValue) {
        ...
        if (scope.typeSpecification) {
          const propertiesElem = angular.element(element[0].querySelector('.funding-new-case-permissions-properties'));
          propertiesElem.html(scope.typeSpecification.template);
          $compile(propertiesElem.contents())(scope);
        }
      });
    },
    templateUrl: '~/crmFunding/program/newCasePermissions/fundingNewCasePermissions.template.html',
