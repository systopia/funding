 ...
 * extension.
 */
class CRM_Funding_ExtensionUtil {
  const SHORT_NAME = 'funding';
  const LONG_NAME = 'funding';
  const CLASS_PREFIX = 'CRM_Funding';
...
spl_autoload_register('_funding_civix_class_loader', TRUE, TRUE);

function _funding_civix_class_loader($class) {
  if ($class === 'CRM_Funding_DAO_Base') {
    if (version_compare(CRM_Utils_System::version(), '5.74.beta', '>=')) {
      class_alias('CRM_Core_DAO_Base', 'CRM_Funding_DAO_Base');
      // ^^ Materialize concrete names -- encourage IDE's to pick up on this association.
    }
    else {
      $realClass = 'CiviMix\\Schema\\Funding\\DAO';
  ...
  if (strpos($class, 'CiviMix\\Schema\\Funding\\') === 0) {
 ...
 * @return bool
 */
function _funding_civix_insert_navigation_menu(&$menu, $path, $item) {
  ...
  if (empty($path)) {
    ...
    foreach ($menu as $key => &$entry) {
      if ($entry['attributes']['name'] == $first) {
        ...
        $found = _funding_civix_insert_navigation_menu($entry['child'], implode('/', $path), $item);
      }
    }
    return $found;
  }
 ...
 */
function _funding_civix_navigationMenu(&$nodes) {
  if (!is_callable(['CRM_Core_BAO_Navigation', 'fixNavigationMenu'])) {
    _funding_civix_fixNavigationMenu($nodes);
  }
}
 ...
 * missing them.
 */
function _funding_civix_fixNavigationMenu(&$nodes) {
  $maxNavID = 1;
  ...
  _funding_civix_fixNavigationMenuItems($nodes, $maxNavID, NULL);
}

function _funding_civix_fixNavigationMenuItems(&$nodes, &$maxNavID, $parentID) {
