 ...
 * @covers \Civi\Funding\Api4\Action\FundingClearingProcess\SetCalculativeReviewerAction
 * @covers \Civi\Funding\ClearingProcess\Api4\ActionHandler\SetCalculativeReviewerActionHandler
 *
 * @group headless
 */
final class SetCalculativeReviewerActionTest extends AbstractFundingHeadlessTestCase {
  ...
  public function test(): void {
    ...
    $e = NULL;
    try {
      FundingClearingProcess::setCalculativeReviewer()
        ->setClearingProcessId($this->clearingProcessBundle->getClearingProcess()->getId())
        ->setReviewerContactId($contact['id'])
        ->execute();
    }
    catch (UnauthorizedException $e) {
      static::assertSame('Permission to change calculative reviewer is missing.', $e->getMessage());
    }
    ...
    static::assertSame($contact['id'], FundingClearingProcess::get(FALSE)
      ->addSelect('reviewer_calc_contact_id')
      ->addWhere('id', '=', $this->clearingProcessBundle->getClearingProcess()->getId())
      ->execute()->single()['reviewer_calc_contact_id']
    );
    ...
    try {
      ...
      static::assertSame(
        sprintf('Contact %d is not allowed as calculative reviewer.', $anotherContact['id']),
        $e->getMessage()
      );
    }
    static::assertNotNull($e);
