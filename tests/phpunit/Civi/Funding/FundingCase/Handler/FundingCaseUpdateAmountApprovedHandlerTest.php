...
/**
 * @covers \Civi\Funding\FundingCase\Handler\FundingCaseUpdateAmountApprovedHandler
 * @covers \Civi\Funding\FundingCase\Command\FundingCaseApproveCommand
 */
final class FundingCaseUpdateAmountApprovedHandlerTest extends TestCase {
  ...
  private FundingCaseUpdateAmountApprovedHandler $handler;
  ...
  protected function setUp(): void {
    ...
    $this->handler = new FundingCaseUpdateAmountApprovedHandler(
  ...
  public function testHandle(): void {
    ...
    $this->statusDeterminerMock->method('getStatus')
      ->with($command->getFundingCase()->getStatus(), 'update-amount-approved')
  ...
  public function testHandleUnauthorizedActionNotAllowed(): void {
    ...
    $this->expectExceptionMessage('Updating the approved amount of this funding case is not allowed.');
    $this->handler->handle($command);
  }
  ...
  private function createCommand(): FundingCaseUpdateAmountApprovedCommand {
    $fundingCaseBundle = FundingCaseBundleFactory::create();
    $amount = 12.34;

    return new FundingCaseUpdateAmountApprovedCommand(
      $fundingCaseBundle,
      $amount,
